...
num_case_model_train_list = [300000]
for num_case_model_train in num_case_model_train_list:
    ...
    generate_result_json = jsonlines.open("mix_data/test/test_mix_total_" + str(num_case_model_train) + "_result.jsonl", mode = "r")
    ...
    for generate, correct in zip(generate_result_json, correct_result_json):
        result_obj = {}
        result_obj["generate"] = generate["script"]

        generate_result_char = re.sub(r'\s', '',generate["script"])
        ...
        if generate_result_char != correct_result_char:
            ...
            verify_result = verify_lua_script(generate["script"])
            if verify_result == False:
                result_obj["verify_result"] = False
            else:
                if check_indentation(generate["script"], correct_result.group(1)):
    ...
    with open("mix_data/test/test_mix_total_" + str(num_case_model_train) + "_check.json", "w", encoding='utf-8') as f:
