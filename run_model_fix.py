...
for num_test_case in num_test_case_list:
    ...
    checkpoint = "finetune_starcoder2_fix_mini_" + str(num_test_case) + "/final_checkpoint"
    ...
    cj = jsonlines.open("fix_data/test/fix_test_1000.jsonl", mode = "r")
    wf = jsonlines.open("fix_data/test/fix_test_" + str(num_test_case) + "_result.jsonl", mode = "w")
        ...
        if len(script_line) < inputs_str_line_num:
            ...
            if script_line[inputs_str_line_num + 1] != "/correct_script":
                ...
                if script == None:
                    ...
                    script = re.search(f"{output_start_str}(.*?){output_end_str}", script, re.DOTALL)
                    if script == None:
                        outputs = model.generate(inputs, int(inputs.shape[1] * 2.5), eos_token_id=eos_token_id, pad_token_id=pad_token_id, do_sample=False, temperature=0.75)
                        script = tokenizer.decode(outputs[0])
                        script = re.search(f"{output_start_str}(.*?){output_end_str}", script, re.DOTALL)
                        if script == None:
