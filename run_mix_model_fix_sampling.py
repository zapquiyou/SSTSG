...
num_test_case_list = [15000]
temperature_list = [0.1, 0.2, 0.25, 0.5]
...
for num_test_case in num_test_case_list:
    ...
    for loop in range(10):
        for temperature in temperature_list:
            ...
            cj = jsonlines.open("mix_data/test/fix_test_1000.jsonl", mode = "r")
            wf = jsonlines.open("mix_data/test/test_mix_fix_" + str(num_test_case) + "_temperature_" + str(temperature) + "_" + str(loop) + "_result.jsonl", mode = "w")

            index = 0
            input_start_str = "/fix\n"
            input_end_str = "/fix_end\n"
            output_start_str = "/correct_script\n"
            output_end_str = "/correct_script_end\n"
            for input_scripts in cj:
                ...
                outputs = model.generate(inputs, max_length=int(inputs.shape[1] * 1.5), eos_token_id=eos_token_id, pad_token_id=pad_token_id, do_sample=True, temperature=temperature)
                ...
                if len(script_line) < inputs_str_line_num:
                    ...
                    if script_line[inputs_str_line_num + 1] != "/correct_script":
                        ...
                        if script == None:
                            outputs = model.generate(inputs, max_length=int(inputs.shape[1] * 2), eos_token_id=eos_token_id, pad_token_id=pad_token_id, do_sample=True, temperature=temperature)
                            script = tokenizer.decode(outputs[0])
                            script = re.search(f"{output_start_str}(.*?){output_end_str}", script, re.DOTALL)
                            if script == None:
                                outputs = model.generate(inputs, max_length=int(inputs.shape[1] * 2.5), eos_token_id=eos_token_id, pad_token_id=pad_token_id, do_sample=True, temperature=temperature)
                                script = tokenizer.decode(outputs[0])
                                script = re.search(f"{output_start_str}(.*?){output_end_str}", script, re.DOTALL)
                                if script == None:
